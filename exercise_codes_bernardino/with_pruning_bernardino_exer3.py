...
class BoardState:
    def __init__(self, dimension = 3):
        ...
        # game state
        self.moves = 0
        self.max_moves = dimension * dimension # for determining draws
    ...
    # function to check the winner quietly, returning the winning symbol or nothing
    def get_winner(self):
        # check rows
    ...
    # minmax algorithm to compite the best move
    def minmax(self, alpha = -math.inf, beta = math.inf):
        self.node_count += 1
        # check if there is a winner
        winner = self.get_winner()

        if winner != None or self.moves == 9:
            if winner == "X":
                return 1    # maximizer is X
            elif winner == "O":
                return -1   # minimizer is O
            else:
                return 0    # draw
...
print("Prunes:", state.prune_count)
